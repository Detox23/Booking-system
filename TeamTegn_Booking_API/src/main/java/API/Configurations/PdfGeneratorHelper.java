...
public class PdfGeneratorHelper {
    ...
    public void generatePDFFromHTML(int id) throws IOException, DocumentException {
        AssignmentEntity assignmentEntity = (AssignmentEntity)this.assignmentDAO.findById(id).get();
        ServiceUserEntity userEntity = (ServiceUserEntity)this.serviceUserDAO.findById(assignmentEntity.getServiceUserId()).get();
        AssignmentTitleEntity assignmentTitleEntity = (AssignmentTitleEntity)this.titleDAO.findById(assignmentEntity.getAssignmentTitle()).get();
        ServiceUserAccountEntity serviceUserAccountEntity = (ServiceUserAccountEntity)this.serviceUserAccountsDAO.findById(assignmentEntity.getServiceUserAccountId()).get();
        AccountEntity accountEntity = (AccountEntity)this.accountDAO.findById(serviceUserAccountEntity.getAccountId()).get();
        List<AssignmentServiceProviderEntity> serviceProviderList = this.providerDAO.findAllByAssignmentIdIs(assignmentEntity.getId());
        AssignmentTypeEntity assignmentTypeEntity = (AssignmentTypeEntity)this.assignmentTypeDAO.findById(assignmentEntity.getAssignmentTitle()).get();
        if (assignmentEntity != null && userEntity != null && assignmentTitleEntity != null && serviceUserAccountEntity != null && accountEntity != null && serviceProviderList != null && assignmentTypeEntity != null) {
            ...
            String content = Files.toString(new File("P:\\Booking-systemfinal\\TeamTegn_Booking_API\\src\\main\\java\\API\\HTML/Report.html"), Charset.defaultCharset());
